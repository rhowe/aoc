...
draw() {
  while read -r char; do
    case "$char" in
      10)
        [ "$width" -ne 0 ] || width=${#map}
        (( height++ )) || :
        ;;
      60|62|94|118)
        robot=($((${#map} % width)) $((${#map}/width)))
        map="$map$(printf \\x$(printf %x "$char"))"
        ;;
      *) map="$map$(printf \\x$(printf %x "$char"))" ;;
    esac
  done
  ...
  for (( row=0; row < height; row++)); do
    echo "${map:row*width:width}"
  done
  echo robot at x=${robot[0]}, y=${robot[1]}
}
